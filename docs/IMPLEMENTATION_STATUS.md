...
- 当前阶段：阶段一至阶段四已完成；欧美日期码规则已完成 FNOS 实例更新，等待用户执行验证扫描。
- FNOS 边界：已按用户授权更新 Docker 镜像并重建容器；未执行远程扫描、媒体删除或数据清理。
...
- 日本系列与 FC2 规则未改动；FNOS 已部署本次新规则，但尚未执行新规则扫描、写入或删除。

### 欧美版本优先级与结果页修复 ✅

- 日期码解析优先识别 `YY.MM.DD`，不会再把 `-4K` 后缀误识别为场景码的一部分。
- 欧美版本优先级改为：`4K+字幕` ＞ `4K`/`字幕` ＞ 有封面普通版 ＞ 无封面普通版；路径版本信息可在 STRM 缺少媒体质量字段时参与选优。
- 结果页分页改为按完整查重组分桶，修复跨页重复/融合；删除或忽略后按真实接口结果更新页面。
- 删除对象明确为 Emby 条目：即使条目 source_type 为 STRM，也由 Emby API 执行删除；应用不删除 STRM 或远程源文件。若未来请求源文件对象，前端保留条目并显示拒绝原因。

### FNOS 部署验证 ✅

- 使用 `/vol1/1000/docker_data/docker-compose.yml` 原配置拉取并重建 `emby-clean`，保留原有 `/data` 挂载、端口和 Compose 管理方式。
- 容器镜像 revision：`a01c55418422148ab3c41218582e7936425693f5`；容器状态 `running/healthy`。
- `/api/health` 返回 `ok=true`、`version=2.1.0`；SQLite `quick_check=ok`，`media_items=65356`，确认现有数据未丢失。
- 未自动触发扫描和删除，下一步由用户在前端执行一次欧美只读查重验证。
...
- 删除队列只保存 Emby 条目，worker 通过 Emby API 发起删除并确认条目消失；不会直接删除 STRM 或远程源文件。定时自动删除默认关闭，必须单独打开高风险开关。
...
- 保留风险：当前“回滚”边界是 Emby 条目删除前的计划和审计，未实现文件系统回收站；STRM/远程源文件仍禁止由本应用直接删除。
...
- FNOS 生产观察：已部署镜像并通过健康检查；未触发同步、扫描或删除，等待用户验证结果。
